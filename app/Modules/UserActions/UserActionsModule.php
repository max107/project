...
class UserActionsModule extends Module
{
    public static function preConfigure()
    {
        ...
        $signal->handler('\Mindy\Auth\AuthProvider', 'onAuth', function (IUser $user) {
            $attrs = [
                'message' => self::t('User <a href="{url}">{name}</a> logged in', [
                    '{url}' => $user->getAbsoluteUrl(),
                    '{name}' => (string)$user
                ]),
